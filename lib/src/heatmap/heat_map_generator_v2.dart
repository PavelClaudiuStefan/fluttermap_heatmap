...
class HeatMapV2 {
  ...
  Future<ui.Image> _grayscaleHeatmap(List<DataPoint> data, ui.Image baseCircle) async {
    final recorder = ui.PictureRecorder();
    final canvas = Canvas(recorder);
  ...
  Future<Uint8List> generate(List<DataPoint> data) async {
    await ready.future;

    // if there is no data then return a transparent image
    if (data.isEmpty) return kTransparentImage;

    // generate shape to be used for all points on the heatmap
    final baseShape = await _getBaseShape();

    final grayscale = await _grayscaleHeatmap(data, baseShape);
